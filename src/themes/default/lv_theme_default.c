...
static void style_init(void)
{
    ...
    lv_theme_add_component(&theme, LV_SCREEN_ID);
    lv_theme_add_style(&theme, LV_SCREEN_ID, &styles->scr, 0);
    lv_theme_add_style(&theme, LV_SCREEN_ID, &styles->scrollbar, LV_PART_SCROLLBAR);
    lv_theme_add_style(&theme, LV_SCREEN_ID, &styles->scrollbar_scrolled, LV_PART_SCROLLBAR | LV_STATE_SCROLLED);

    lv_theme_add_component(&theme, LV_SCREEN_LAYER_ID);

    lv_theme_add_component(&theme, LV_OBJ_ID);
    lv_theme_add_style(&theme, LV_OBJ_ID, &styles->card, 0);
    lv_theme_add_style(&theme, LV_OBJ_ID, &styles->scrollbar, LV_PART_SCROLLBAR);
    lv_theme_add_style(&theme, LV_OBJ_ID, &styles->scrollbar_scrolled, LV_PART_SCROLLBAR | LV_STATE_SCROLLED);


    lv_theme_add_component(&theme, LV_BTN_ID);
    lv_theme_add_style(&theme, LV_BTN_ID, &styles->btn, 0);
    lv_theme_add_style(&theme, LV_BTN_ID, &styles->bg_color_primary, 0);
    lv_theme_add_style(&theme, LV_BTN_ID, &styles->transition_delayed, 0);
    lv_theme_add_style(&theme, LV_BTN_ID, &styles->pressed, LV_STATE_PRESSED);
    lv_theme_add_style(&theme, LV_BTN_ID, &styles->transition_normal, LV_STATE_PRESSED);
    lv_theme_add_style(&theme, LV_BTN_ID, &styles->outline_primary, LV_STATE_FOCUS_KEY);
#if LV_THEME_DEFAULT_GROW
    lv_theme_add_style(&theme, LV_BTN_ID, &styles->grow, LV_STATE_PRESSED);
#endif
    lv_theme_add_style(&theme, LV_BTN_ID, &styles->bg_color_secondary, LV_STATE_CHECKED);
